...
namespace MNN {
  FileLoaderMmap::FileLoaderMmap(const char* file) {
    ...
    mFile = fopen(file, "rb");
    if(mFile == NULL){
      MNN_PRINT("%s:%d:%s: Error No: %d, open file failed %s\n", __FILE__, __LINE__, __FUNCTION__,  errno, file);
    }

#endif
    std::fseek(mFile, 0, SEEK_END);
    size_t size = std::ftell(mFile);
    mFileSize =  size;
...
#ifdef MNN_MMAP
    auto block = MemoryAllocAlignMmap(gCacheSize, "test1.txt", MNN_MEMORY_ALIGN_DEFAULT);
#else
...
#ifdef MNN_MMAP
      block = MemoryAllocAlignMmap(gCacheSize, "test1.txt", MNN_MEMORY_ALIGN_DEFAULT);
#else
